...
export async function handler(event, context) {
  try {
    ...
    console.error('[server] handler error:', err?.message || err, err?.stack);
    const body = JSON.stringify({
      error: 'Server-Fehler (Function).',
      detail: (err?.message && String(err.message).slice(0, 200)) || 'Unbekannter Fehler – siehe Netlify Function-Log.',
    });
    return {
      statusCode: 500,
